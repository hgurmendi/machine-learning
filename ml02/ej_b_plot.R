...
if (length(args) != 2) {
    message("USAGE: ./ej_b_plotter.R file_stem data|prediction")
    quit()
}
...
} else if (extension == "prediction") {
    data <- read.table(dataFile, header=FALSE)
} else {
    message("File extension not supported. It has to be either 'data' or 'prediction'.")
    quit()
}
