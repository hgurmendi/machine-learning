...
/*ALLOCAR ESPACIO PARA LAS MATRICES DEL ALGORITMO*/
    mus = (double **) malloc(N_Class * sizeof(double *));
    if (mus == NULL) {
        return 1;
    }
    for (i = 0; i < N_Class; i++) {
    ...
    variances = (double **) malloc(N_Class * sizeof(double *));
    if (variances == NULL) {
        return 1;
    }
    for (i = 0; i < N_Class; i++) {
        variances[i] = (double *) calloc(N_IN, sizeof(double));
        if (variances[i] == NULL) {
            return 1;
        }
    }
    ...
    histogram = (Bin ***) malloc(N_Class * sizeof(Bin **));
    if (histogram == NULL) {
        return 1;
    }
    for (i = 0; i < N_Class; i++) {
        histogram[i] = (Bin **) malloc(N_IN * sizeof(Bin *));
        if (histogram[i] == NULL) {
            return 1;
        }
        for (j = 0; j < N_IN; j++) {
            histogram[i][j] = (Bin *) calloc(N_BINS, sizeof(Bin));
...
double prob(double x, int feature, int clase)
{
    ...
    for (i = 0; i < N_BINS; i++) {
        if (x < histogram[clase][feature][i].max) {
            break;
        }
    }
    
    return (histogram[clase][feature][i].count + 1.0) / (double) (cant_clase[clase] + N_BINS);
}
...
int output(double *input)
{
    ...
    for (k = 0; k < N_Class; k++) {
        ...
        prob_de_clase += log(((double) cant_clase[k])/PR);
...
int train(char *filename)
{
    ...
    for (i = 0; i < N_IN; i++) {
        min = max = data[seq[0]][i];

        /* Get the min and max value of the current feature */
        for (j = 0; j < N_TOTAL; j++) {
            clase = (int) data[seq[j]][N_IN];
        ...
        for (j = 0; j < N_Class; j++) {
            for (k = 0; k < N_BINS; k++) {
                histogram[j][i][k].size = size;
                histogram[j][i][k].min = min + k * size;
                histogram[j][i][k].max = histogram[j][i][k].min + size;
                histogram[j][i][k].count = 0;

                printf("histogram[%02d][%02d][%02d]: (%f - %f)\n", j, i, k, histogram[j][i][k].min, histogram[j][i][k].max);
            }
        }
    }

    for (i = 0; i < N_TOTAL; i++) {
        clase = (int) data[seq[i]][N_IN];

        for (j = 0; j < N_IN; j++) {
            for (k = 0; k < N_BINS; k++) {
                if (data[seq[i]][j] < histogram[clase][j][k].max) {
                    break;
                }
            }

            histogram[clase][j][k].count++;
        }
    }
    ...
    for (i = 0; i < N_Class; i++) {
        for (j = 0; j < N_IN; j++) {
            for (k = 0; k < N_BINS; k++) {
                printf("histogram[%02d][%02d][%02d]\t%d\n", i, j, k, histogram[i][j][k].count);
            }
        }
    }
